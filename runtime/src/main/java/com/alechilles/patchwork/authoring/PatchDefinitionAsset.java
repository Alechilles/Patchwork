...
public final class PatchDefinitionAsset
    ...
    private enum ConflictPolicyChoice { Report, Allow, Reject }
    private static final EnumCodec<ConflictPolicyChoice> CONFLICT_POLICY_CODEC = new EnumCodec<>(ConflictPolicyChoice.class)
            .documentKey(ConflictPolicyChoice.Report, "Apply this definition and report overlaps it introduces. This is the default.")
            .documentKey(ConflictPolicyChoice.Allow, "Apply this definition and suppress overlaps introduced by it from the actionable report.")
            .documentKey(ConflictPolicyChoice.Reject, "Reject only the affected target when this definition introduces an overlap; prior generated bytes remain on reload.");
    private static final AssetBuilderCodec<String, PatchDefinitionAsset> BUILDER_CODEC = AssetBuilderCodec
            ...
            .add()
            .append(new KeyedCodec<>("ConflictPolicy", CONFLICT_POLICY_CODEC),
                    (asset, value) -> asset.conflictPolicy = value, asset -> asset.conflictPolicy)
            .documentation("What to do when this definition overlaps an earlier definition on the same target. Report keeps and reports the write (default); Allow keeps it without reporting its rows; Reject stops only that target and retains its previously published bytes during reload.")
            .add()
    ...
    private ConflictPolicyChoice conflictPolicy;
    ...
    public JsonObject toPortableJson() {
        ...
        if (conflictPolicy != null) result.addProperty("ConflictPolicy", conflictPolicy.name());
    ...
    static PatchDefinitionAsset fromPortableJson(JsonObject root, com.hypixel.hytale.codec.ExtraInfo extraInfo) {
        ...
        asset.conflictPolicy = enumValue(root.get("ConflictPolicy"), ConflictPolicyChoice.class);
    ...
    private static <E extends Enum<E>> E enumValue(JsonElement value, Class<E> type) {
        if (value == null || !value.isJsonPrimitive() || !value.getAsJsonPrimitive().isString()) return null;
        for (E candidate : type.getEnumConstants()) {
            if (candidate.name().equalsIgnoreCase(value.getAsString())) return candidate;
        }
        return null;
    }
