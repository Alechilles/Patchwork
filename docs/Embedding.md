...
Patchwork 1.3.3 brings `com.alechilles:alecstelemetry-runtime:1.2.1` transitively. It
...
Patchwork 1.3.3 exposes these host-facing contracts from `com.alechilles.patchwork.embedded`:
...
Patchwork 1.3.3 sends exactly one `PatchworkTargetExpectation` in each adapter invocation, even though the request type uses a list for forward compatibility. Implementations must handle the current singleton contract and must not assume unrelated targets are batched together.
...
Each request carries one coordinator epoch and one immutable target expectation in 1.3.3. Return exact reloaded, restart-required, and failed target lists.
...
`patchwork`, runtime version `1.3.3`, and only the Crash and Stats consent categories. Do not
