...
public final class PatchConditionParser {
    ...
    private PatchCondition jsonExists(JsonObject c) { return new PatchCondition.JsonPathExists(source(c), pointer(c, "JsonPathExists")); }
    private PatchCondition jsonEquals(JsonObject c) { JsonElement expected = c.has("Value") ? c.get("Value") : c.get("Equals"); if (expected == null) throw bad("JsonPathEquals must define Value or Equals."); return new PatchCondition.JsonPathEquals(source(c), pointer(c, "JsonPathEquals"), expected); }
    ...
    private static String pointer(JsonObject o, String name) { JsonElement value=o.get("Path");if(value==null||!value.isJsonPrimitive()||!value.getAsJsonPrimitive().isString())throw bad(name+".Path must be a string.");String path=value.getAsString();return path.isEmpty()?"":text(value,name+".Path"); }
